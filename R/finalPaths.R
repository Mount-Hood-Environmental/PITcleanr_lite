...
finalPaths = function(obs_direct = obs_direct){
  ...
  out = obs_direct %>%
    ...
    left_join(obs_direct %>%
                ...
                mutate(interior_use_hr = ifelse(restoration_interior == "Yes", residency_hr,0),
                       travel_use_hr = ifelse(slot > 1 & lag(restoration_entrance_or_exit) == "Yes" |
                                                slot > 1 & lag(restoration_interior) == "Yes", travel_time_hr, 0)) %>%
